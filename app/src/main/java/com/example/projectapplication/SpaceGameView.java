...
public class SpaceGameView extends SurfaceView implements Runnable, SensorEventListener {
    ...
    private static final float SPEED_SCALE_FACTOR = 0.999f;
    ...
    public SpaceGameView(Context context, int x, int y) {
        ...
        level = 1;
        lives = getResources().getInteger(R.integer.livesNumber);
    ...
    public void playMusic(){
            musicPlayer = MediaPlayer.create(context, R.raw.menumusic);
            musicPlayer.start();
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            // Lấy giá trị gia tốc trên trục X và trục Y
            float accelX = -event.values[0];
            float accelY = -event.values[2];

            float deltaX = accelX * SPEED_SCALE_FACTOR;
            // Tính toán delta cho cả hai trục
            float deltaY = accelY * SPEED_SCALE_FACTOR;
